...
def navigate_wait_with_cv(x=0, y=0, z=0, yaw=float('nan'), speed=0.5, frame_id='aruco_map', auto_arm=False, tolerance=0.2):
    navigate(x=x, y=y, z=z, yaw=yaw, speed=speed, frame_id=frame_id, auto_arm=auto_arm)
...
def navigate_wait_fixed(x=0, y=0, z=1.5, yaw=float('nan'), speed=0.7, frame_id='aruco_map', auto_arm=False, tolerance=0.2):
    global platform_found
    navigate(x=x, y=y, z=z, yaw=yaw, speed=speed, frame_id=frame_id, auto_arm=auto_arm)
...
def main():
    ...
    rospy.sleep(0.5)
    set_effect(g=255, effect='rainbow')


    markers = ['aruco_36', 'aruco_39' ]
    k = 0

    while True:
        print(markers[k])
        navigate_wait_with_cv(z=z, speed=1, frame_id=markers[k])
        if platform_found:
            rospy.sleep(3)
            break
        if k  ==1:
    ...
    while d > 0.08: # or (rospy.get_time() - st_t < 0.5): 
        print("move")
        print("d", d ,  "st_t", rospy.get_time() - st_t)
        ...
        if prev_pa is None:
            if pb is None:
                r.sleep()
                continue
            set_effect(b=255, effect='fill')

            navigate(x=pb[0], y=pb[1], z=0.5, speed=1, frame_id="aruco_map") # Снижение высоты к подлету к маркеру
            prev_pa = pa
            prev_t = now
            ...
            if pa is not None:
                ...
                set_position(x=t[0], y=t[1], z=0.5, frame_id="aruco_map")
                prev_pa = pa.copy()
                prev_vel = vel.copy()
                prev_t = now
            else:
                navigate(x=pb[0], y=pb[1],speed= 1, z=0.5, frame_id="aruco_map")
                print("NO pa and vel")
