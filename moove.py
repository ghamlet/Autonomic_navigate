...
def navigate_wait_fixed(x=0, y=0, z=1, yaw=float('nan'), speed=0.7, frame_id='aruco_map', auto_arm=False, tolerance=0.2):
...
def main():
    ...
    for marker in ['aruco_40', 'aruco_36', 'aruco_29', 'aruco_33']:
        navigate_wait(z=1, speed=1, frame_id=marker)
        if platform_found:
            break
    ...
    while d > 0.08 or (rospy.get_time() - st_t < 0.5): 
        ...
        if prev_pa is None:
            ...
            navigate(x=pb[0], y=pb[1], z=z, speed=1.5, frame_id="aruco_map") # Снижение высоты к подлету к маркеру
            prev_pa = pa
            prev_t = now
        else:
            if pb is not None:
            ...
            if pa is not None:
                ...
                navigate(x=pb[0], y=pb[1],speed= 1.5, z=z, frame_id="aruco_map")
                print("NO pa and vel")
